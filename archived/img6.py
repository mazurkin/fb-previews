...
def filter_fft(img, band=12, steps=16):
    ...
    if steps > 1:
        c_fft_mod_r_min = c_fft_mod.real.min()
        c_fft_mod_r_max = c_fft_mod.real.max()
        c_fft_mod_r_stp = (c_fft_mod_r_max - c_fft_mod_r_min) / (steps - 1)
        if c_fft_mod_r_stp != 0:
            # compact
            c_fft_mod.real = np.round(c_fft_mod.real / c_fft_mod_r_stp)
            # restore
            c_fft_mod.real = np.multiply(c_fft_mod_r_stp, c_fft_mod.real)

        c_fft_mod_i_min = c_fft_mod.imag.min()
        c_fft_mod_i_max = c_fft_mod.imag.max()
        c_fft_mod_i_stp = (c_fft_mod_i_max - c_fft_mod_i_min) / (steps - 1)
        if c_fft_mod_i_stp != 0:
            # compact
            c_fft_mod.imag = np.round(c_fft_mod.imag / c_fft_mod_i_stp)
            # restore
            c_fft_mod.imag = np.multiply(c_fft_mod_i_stp, c_fft_mod.imag)
